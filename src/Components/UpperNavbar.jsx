	...
	return (
		<>
			...
			<div className="text-center p-[3px] bg-[#f14D54] text-white">
				Get{" "}
				<span className="text-black font-bold bg-slate-300 p-[2px] rounded-lg">
					20% Off
				</span>{" "}
				on your First order
			</div>
			...
			<div className="flex justify-between items-center px-[40px]">
				...
				<div className="flex text-black  gap-2">
					{!user ? (
						...
						<>
							<Link to="/updateprofile" className="authbtn">
								Update Profile
							</Link>
							{user.roleId == 2 ? (
								<>
									<hr className="w-[2px] bg-red-500 h-8" />
									<Link to="/books" className="authbtn">
										Books
									</Link>
								</>
							) : user.roleId == 1 ? (
								<>
									...
									<hr className="w-[2px] bg-red-500 h-8" />
									<Link to="/category" className="authbtn">
										Category
									</Link>
								</>
							) : null}
					...
					>
						<h1 className="bg-blue-200 px-2 py-1">
							{user ? user.firstName + " " + user.lastName : null}
						</h1>

						<div
							className="bg-red-300/60 mx-2 px-2 py-1 hover:text-white hover:bg-red-400 border-2 border-red-600 rounded-lg cursor-pointer"
							onClick={Logout}
						>
							Logout
						</div>
